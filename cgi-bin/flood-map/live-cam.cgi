...
my @file_name = &Check_point("index", $north, $south, $east, $west);
...
&Read_kml_template("$path/live_cam/template/head_temp.kml"); 


foreach my $kml (@file_name){
	&Read_kml_template("$path/live_cam/kml/$kml"); 
	#&Read_kml("$kml"); 
}

&Read_kml_template("$path/live_cam/template/tail_temp.kml"); 
...
sub Read_kml{
	my ($file) = @_;
  #system("tar zxOf /home/flood-map/live_cam/kml.tgz kml/$file");
  system("tar xOf /home/flood-map/live_cam/kml.tar kml/$file");
  #my @file = split(/\//, $tmp);
  #system("tar zxOf /home/flood-map/live_cam/kml/$file[0].tgz $file[0]/$file[1]");
...
sub Check_square{
	...
	my @files;
	open(FP, "<$path/live_cam/point/$file");
	while(my $tmp=<FP>){
...
sub Check_latlng{
	...
	if($latlng !~ /^-{0,1}\d+\.{0,1}\d*$/){
		...
		print "\n";
		print "format error.\n";
		print "example: http://pingineer.net/cgi-bin/flood-map/live-cam.cgi?north=35.62752873025489&east=134.44981870385743&south=35.61062637667083&west=134.4089632961426&zoom=16\n";
		exit 0;
	}
}
