...
&Read_kml_template("$path/rainfall/template/head_temp.kml"); 


foreach my $kml (@file_name){
	#&Read_kml("$kml"); 
	&Read_kml_template("$path/rainfall/kml/$kml"); 
}

&Read_kml_template("$path/rainfall/template/tail_temp.kml"); 
...
sub Check_square{
	...
	my @files;
	open(FP, "<$path/rainfall/point/$file");
	while(my $tmp=<FP>){
...
sub Check_latlng{
	...
	if($latlng !~ /^-{0,1}\d+\.{0,1}\d*$/){
		...
		print "\n";
		print "format error.\n";
		print "example: http://pingineer.net/cgi-bin/flood-map/rainfall.cgi?north=35.62752873025489&east=134.44981870385743&south=35.61062637667083&west=134.4089632961426\n";
		exit 0;
	}
}
