...
&Read_kml_template("$path/rainfall_overlay/template/head_temp.kml"); 


foreach my $kml (@file_name){
	&Read_kml_template("$path/rainfall_overlay/kml/$kml"); 
}

&Read_kml_template("$path/rainfall_overlay/template/tail_temp.kml"); 
...
sub Check_square{
	...
	my @files;
	open(FP, "<$path/rainfall_overlay/square/$file");
	while(my $tmp=<FP>){
