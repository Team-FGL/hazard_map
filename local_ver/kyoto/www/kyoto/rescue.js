...
function Delete_kyoto_rescue() {
	//console.log(kyoto_rescue_markers.length);
	// 画面外のmarker等は消す
	for(var i=0; i<kyoto_rescue_markers.length; i++){
		if(kyoto_rescue_markers[i].ext.keep_flg == 1){
			// keep_flgが1なら0に戻して次の処理に備える
			kyoto_rescue_markers[i].ext.keep_flg = 0;
		} else {
			// keep_flgが0ならmarkerの配列等を削除
			delete kyoto_rescue_markers[i].ext;
			kyoto_rescue_markers[i].setMap(null);
			kyoto_rescue_markers.splice(i,1);
			i--;
		}
	}
}
...
function Make_kyoto_rescue(lat, lng, name, address,tel,post_code) {
	// 同じIDのmarkerがあればスキップ
	for(var i =0; i<kyoto_rescue_markers.length; i++){
		if(kyoto_rescue_markers[i].ext.name == name){
			kyoto_rescue_markers[i].ext.keep_flg =1;
			return;
		}
	}
	...
	delete marker;
	kyoto_rescue_markers[i].ext=new const_rescue(name,address,tel,post_code,1);
...
function const_rescue(name,address,tel,post_code,keep_flg){
	this.name = name;
