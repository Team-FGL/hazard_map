...
function Delete_kyoto_ess() {
	//console.log(kyoto_ess_markers.length);
	// 画面外のmarker等は消す
	for(var i=0; i<kyoto_ess_markers.length; i++){
		if(kyoto_ess_markers[i].ext.keep_flg == 1){
			// keep_flgが1なら0に戻して次の処理に備える
			kyoto_ess_markers[i].ext.keep_flg = 0;
		} else {
			// keep_flgが0ならmarkerの配列等を削除
			delete kyoto_ess_markers[i].ext;
			kyoto_ess_markers[i].setMap(null);
			kyoto_ess_markers.splice(i,1);
			i--;
		}
	}
}
...
function Make_kyoto_ess(lat, lng, name, address,tel,post_code) {
	// 同じIDのmarkerがあればスキップ
	for(var i =0; i<kyoto_ess_markers.length; i++){
		if(kyoto_ess_markers[i].ext.name == name){
			kyoto_ess_markers[i].ext.keep_flg =1;
			return;
		}
	}
	...
	var marker = new google.maps.Marker({
		position: myLatlng,
		map: map,
		icon: "http://maps.google.co.jp/mapfiles/ms/icons/grocerystore.png",
		title:name
	});
	...
	delete marker;
	kyoto_ess_markers[i].ext=new const_ess(name,address,tel,post_code,1);
...
function const_ess(name,address,tel,post_code,keep_flg){
	this.name = name;
